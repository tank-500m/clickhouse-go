...
* connection_open_strategy - random/round_robin/in_order (default in_order).
    * random      - choose random server from the set; when in-flight connections are below `max_idle_conns` and `max_idle_conns` > 1, a new connection is dialed before reusing idle ones
    * round_robin - choose a round-robin server from the set; when in-flight connections are below `max_idle_conns` and `max_idle_conns` > 1, a new connection is dialed before reusing idle ones
...
* **connection_open_strategy** - Strategy for selecting servers from the connection pool:
  ...
  * `round_robin` - Choose servers in a round-robin fashion; when in-flight connections are below `max_idle_conns` and `max_idle_conns` > 1, a new connection is dialed before reusing idle ones
  * `random` - Choose a random server from the pool; when in-flight connections are below `max_idle_conns` and `max_idle_conns` > 1, a new connection is dialed before reusing idle ones
